...
public class ARController : MonoBehaviour
{
	...
	private GameObject _videoBackgroundFeedbackCameraGO1 = null; // The GameObject which holds the Camera object(s) for the stereo right-eye video background.
	private Camera _videoBackgroundFeedbackCamera1 = null; // The Camera component attached to _videoBackgroundCameraGO1. Easier to keep this reference than calling _videoBackgroundCameraGO1.GetComponent<Camera>() eaach time.
...
//    	AR_MATRIX_CODE_GLOBAL_ID = 2830
        ...
        if (!_sceneConfiguredForVideo) {
            ...
            if (!PluginFunctions.arwIsRunning()) {
				...
				if (!haveStereoARCameras) {
					...
					Debug.Log(_videoBackgroundCameraGO0.GetComponent<Camera>().rect);// = new Rect(0.0f,0.0f,1.0f,1.0f);
					...
					_videoBackgroundFeedbackCameraGO1 = CreateVideoBackgroundCamera("Video Feedback background (R)", (VideoIsStereo ? BackgroundLayer1 : BackgroundLayer0), out _videoBackgroundFeedbackCamera1);
	...
	public bool UseVideoBackground
	{
		...
		set
		{
			...
			if (_videoBackgroundFeedbackCamera1 != null) _videoBackgroundFeedbackCamera1.enabled = currentUseVideoBackground;
		}
	}
	...
	private bool ConfigureViewports()
	{
		...
		foreach (ARCamera arc in arCameras) {
			if (!arc.Stereo) {
				...
				arc.gameObject.GetComponent<Camera>().rect = new Rect(0.0f,0.0f,1.0f,1.0f);
			} else {
				// One eye of a stereo display.
		...
		if (!haveStereoARCamera) {
			...
			Debug.Log(_videoBackgroundCamera0.pixelRect);
			_videoBackgroundCamera0.rect = new Rect(0.0f,0.0f,1.0f,1.0f);
			_videoBackgroundCamera0.targetDisplay = 1;
		} else {
			// Stereo display.
			...
			_videoBackgroundFeedbackCamera1.pixelRect = getViewport(_videoWidth0, _videoHeight0, false, ARCamera.ViewEye.Left);
			_videoBackgroundFeedbackCamera1.targetDisplay = 1;
		}
